...
function MyApplications() {
  ...
  if (view === 'list') {
    return (
      <div>
        {message && (
          <div className={`mb-4 rounded-lg px-4 py-3 text-sm font-medium ${message.type === 'success' ? 'bg-emerald-50 text-emerald-700' : 'bg-rose-50 text-rose-700'}`}>
            {message.text}
          </div>
        )}
        ...
        <div className="mb-4 flex items-center justify-between">
          ...
          <button onClick={() => { setView('form'); setMessage('') }}
            className="rounded-lg bg-[#0077b6] px-4 py-2 text-sm font-batchibold text-white transition hover:bg-indigo-700">
            + New Application
          </button>
        </div>
        ...
        {fetching ? (
          ...
          <div className="rounded-xl border-2 border-dashed border-slate-200 py-16 text-center">
            ...
            <button onClick={() => setView('form')}
              className="mt-4 rounded-lg bg-[#0077b6] px-5 py-2 text-sm font-batchibold text-white hover:bg-indigo-700">
              Submit Application
            </button>
          </div>
        ) : (
          <div className="space-y-3">
            {applications.map((app) => (
              <div key={app._id}
                className="flex cursor-pointer flex-col gap-3 rounded-xl border border-slate-200 bg-white p-4 shadow-sm transition hover:border-[#caf0f8] hover:shadow-md sm:flex-row sm:items-center sm:justify-between"
                onClick={() => openDetail(app)}>
                <div className="flex-1">
                  <div className="flex items-center gap-2">
                    <span className="rounded-md bg-[#e0f7fa] px-2 py-0.5 text-xs font-batchibold text-[#023e8a]">
                      {app.type}
                    </span>
                  </div>
    ...
    return (
      <div className="mx-auto max-w-2xl">
        ...
        <div className="rounded-xl bg-white p-6 shadow-sm">
          ...
          {message && (
            <div className={`mb-4 rounded-lg px-4 py-3 text-sm ${message.type === 'success' ? 'bg-emerald-50 text-emerald-700' : 'bg-rose-50 text-rose-700'}`}>
              {message.text}
            </div>
          )}
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              ...
              <label className="flex cursor-pointer flex-col items-center justify-center rounded-xl border-2 border-dashed border-slate-300 bg-slate-50 px-4 py-8 transition hover:border-indigo-400 hover:bg-[#e0f7fa]">
              ...
              {files.length > 0 && (
                <div className="mt-2 space-y-1">
                  {files.map((f, i) => (
                    <div key={i} className="flex items-center gap-2 rounded-lg bg-[#e0f7fa] px-3 py-2 text-sm">
                      <span>📄</span>
                      <span className="flex-1 truncate font-medium text-[#023e8a]">{f.name}</span>
            ...
            <div className="flex gap-3 pt-2">
              <button type="submit" disabled={loading}
                className="flex-1 rounded-lg bg-[#0077b6] py-2.5 text-sm font-batchibold text-white transition hover:bg-indigo-700 disabled:opacity-60">
      ...
      <div className="mx-auto max-w-2xl">
        ...
        <div className="rounded-xl bg-white p-6 shadow-sm">
          ...
          <div className="mb-5 flex items-start justify-between">
            <div>
              <span className="rounded-md bg-[#e0f7fa] px-2 py-0.5 text-xs font-batchibold text-[#023e8a]">{selected.type}</span>
              ...
              <div className="space-y-2">
                {selected.attachments.map((att, i) => (
                  <a key={i} href={`http://localhost:5000/uploads/${att.fileName}`} target="_blank" rel="noreferrer"
                    className="flex items-center gap-2 rounded-lg border border-slate-200 bg-slate-50 p-3 text-sm transition hover:border-indigo-300 hover:bg-[#e0f7fa]">
                    <span className="text-lg">📄</span>
                    <span className="flex-1 font-medium text-[#023e8a]">{att.originalName}</span>
          ...
          {(selected.adminComment || selected.rejectionReason) && (
            <div className={`rounded-lg p-4 ${selected.status === 'Rejected' ? 'bg-rose-50 text-rose-800' : 'bg-emerald-50 text-emerald-800'}`}>
                ...
                {selected.timeline.map((t, i) => (
                  <div key={i} className="flex gap-3">
                    <div className="flex flex-col items-center">
                      <div className="h-2.5 w-2.5 rounded-full bg-[#e0f7fa]0" />
