...
const EMPTY = { title:'', description:'', icon:'📋', tag:'Service', bgFrom:'#03045e', bgTo:'#0077b6', accent:'#48cae4', order:0 }
...
export default function ManageServices() {
  ...
  const inputCls = 'w-full rounded-xl border border-[#caf0f8] bg-[#f0f9ff] px-3 py-2.5 text-sm outline-none focus:border-[#0077b6] focus:bg-white'
    ...
    <div className="space-y-5">
        ...
        <div className="rounded-2xl border border-[#caf0f8] bg-white p-6 shadow-sm">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="grid gap-4 sm:grid-cols-4">
              ...
              <div><label className={labelCls}>BG From</label><input value={form.bgFrom} onChange={e=>setForm({...form,bgFrom:e.target.value})} className={inputCls} placeholder="#03045e" /></div>
      ...
      {services.length === 0 ? (
        ...
        <div className="space-y-3">
          {services.map(svc => (
            <div key={svc._id} className="flex flex-col gap-3 rounded-2xl border border-[#caf0f8] bg-white p-4 shadow-sm sm:flex-row sm:items-center sm:justify-between">
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    <span className="rounded-full bg-[#e0f7fa] px-2 py-0.5 text-[10px] font-bold text-[#03045e]">{svc.tag}</span>
                    <span className={`rounded-full px-2 py-0.5 text-[10px] font-bold ${svc.isActive?'bg-emerald-100 text-emerald-700':'bg-gray-100 text-gray-500'}`}>
              ...
              <div className="flex flex-wrap gap-2">
                <button onClick={() => toggleActive(svc)}
                  className={`rounded-lg px-3 py-1.5 text-xs font-semibold transition ${svc.isActive?'bg-gray-100 text-gray-600 hover:bg-gray-200':'bg-emerald-100 text-emerald-700 hover:bg-emerald-200'}`}>
                  {svc.isActive ? 'Deactivate' : 'Activate'}
                </button>
                <button onClick={() => handleEdit(svc)}
                  className="rounded-lg bg-[#e0f7fa] px-3 py-1.5 text-xs font-semibold text-[#0077b6] transition hover:bg-[#caf0f8]">
                  ✏️ Edit
                </button>
