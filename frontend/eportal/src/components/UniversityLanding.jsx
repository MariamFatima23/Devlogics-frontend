    ...
    <section className="relative overflow-hidden bg-slate-950 px-6 py-20 text-white">
      ...
      <div className="relative mx-auto max-w-7xl">
        <div className="grid gap-10 lg:grid-cols-[1.1fr_0.9fr] lg:items-center">
          <div>
            <div className="mb-4 inline-flex rounded-full border border-white/20 bg-white/10 px-4 py-2 text-sm font-batchibold backdrop-blur">
              Student Portal
            </div>
            <h1 className="mb-5 text-4xl font-extrabold leading-tight md:text-6xl">
              A smart digital portal for <span className="text-cyan-300"> services</span> and student success.
            </h1>
            ...
            <div className="flex flex-wrap gap-3">
              {user ? (
                <Link to="/dashboard" className="rounded-lg bg-white px-6 py-3 font-batchibold text-slate-900 shadow-lg transition hover:scale-105">
                  Open Dashboard
                </Link>
              ) : (
                <>
                  <Link to="/register" className="rounded-lg bg-cyan-400 px-6 py-3 font-batchibold text-slate-950 shadow-lg transition hover:scale-105">
                    Register Now
                  </Link>
                  <Link to="/login" className="rounded-lg border border-white/30 px-6 py-3 font-batchibold text-white transition hover:bg-white/10">
                    Student Login
                  </Link>
                </>
              )}
          ...
          <div className="rounded-3xl border border-white/10 bg-white/10 p-6 shadow-2xl backdrop-blur-md">
            <h2 className="mb-4 text-2xl font-bold">Quick access</h2>
            <div className="space-y-3">
              {quickLinks.map((item) => (
                <Link key={item.title} to={item.href} className="block rounded-2xl border border-white/10 bg-slate-900/40 p-4 transition hover:bg-slate-900/60">
                  <h3 className="font-batchibold text-white">{item.title}</h3>
